...
def analyze():
    ...
    if not file or not jd_text:
        return jsonify({"error": "Resume file aur Job Description dono required hain."}), 400
    ...
    try:
        ...
        if not resume_text.strip():
            return jsonify({"error": "Resume se text extract nahi hua. Dusri file try karo."}), 400
